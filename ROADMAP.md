...
### A. Missing Frontend Capabilities (Current UI is ~15% complete)
...
*   **Threat Intelligence Integration:** Automated ingestion of IOCs from MISP, OpenCTI, or commercial feeds (CrowdStrike, Mandiant).
*   **Reporting & Compliance:** Automated PDF/CSV reports for SOC2, HIPAA, or GDPR compliance audits.
*   **Multi-Cluster Management:** Ability to manage multiple data collectors (Ingestion nodes) from a single central console.
*   **Forensic Artifact Collection:** Triggering remote agents to collect memory dumps or process trees during an investigation.
...
- [ ] **Strict Tenant Isolation:** Enforce mandatory `tenant_id` filtering at the database and search engine layers (per-tenant indices).
...
- [ ] **Investigative Workflow:** Build a "Case Timeline" where analysts can add notes, tags, and evidence (screenshots/logs).
...
- [ ] **Human-in-the-Loop:** Approval workflows for destructive actions (e.g., isolating a server).
